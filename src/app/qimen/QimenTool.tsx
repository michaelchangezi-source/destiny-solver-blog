...
<h1>免費奇門遁甲排盤</h1>
<p class="lead">奇門一時一局：以您起念那一刻的時空，看這件事的局勢與進退。輸入起局時刻，即時排出陰陽遁局數、值符值使與九宮五層盤，並可一鍵複製 AI 解讀資料包，貼至 ChatGPT／Claude／Gemini 深入分析。</p>

<!-- ═══ SECTION:FORM ═══ -->
<section class="card" id="formCard">
  <h2>起局</h2>
  <label for="q">所占之事（選填，一事一局，越具體越好）</label>
  <textarea id="q" maxlength="300" placeholder="例：下星期與客戶簽約，這一單能否成功？"></textarea>
  <div class="row2">
    <div>
...
<section class="card" id="chartCard" style="display:none">
  ...
  <p class="hint">盤圖供您核對；如需 AI 分析，請用下方面板複製資料包。宮內每格由上至下：地盤干／天盤干、九星 · 八神、八門、標記。</p>
</section>

<!-- ═══ SECTION:AI-PANEL ═══ -->
<section class="card" id="aiCard" style="display:none" aria-labelledby="ai-title">
  <h2 id="ai-title">交給 AI 解讀</h2>
  <p class="lead" style="margin-bottom:10px">局排好後，下一步是問對問題。選擇一個方向，複製至您慣用的 ChatGPT／Claude／Gemini，它將依照這個局為您解讀。免費、免登入。</p>
  <div class="chips" role="radiogroup" aria-label="解讀方向" id="chips"></div>
  <p class="blurb" id="blurb"></p>
  <div class="warn" id="freeWarn">請先在上方「所占之事」欄位寫下您想詢問的事項，才能使用自由提問。</div>
  <div class="btns">
  ...
  <div class="privacy">資料包及「重開此局」連結含有您的問事與起局時刻，請僅分享給您信任的人或 AI 服務。</div>
</section>
...
<section class="card">
  <details>
    ...
    <div style="font-size:13.5px;color:var(--muted);margin-top:8px;line-height:1.9">
      ...
      ・邊界：本工具不提供置閏定局、飛盤奇門、真太陽時；奇門流派口徑有異，本站不作「唯一正確」宣稱，資料包寫明口徑，判斷交由解讀者決定。工具只出結構，不出吉凶斷語。</p>
    </div>
  </details>
</section>
...
<section class="seo">
  ...
  <p>請求 AI「幫我起一個奇門局」多數會排錯：模型未必知道今日日辰、目前節氣與拆補三元，值符值使往往全憑推測。正確分工是：由本工具負責排準局數，再複製「Prompt＋資料包」貼至 ChatGPT、Claude 或 Gemini，AI 只負責解讀一份已正確排出的結構化資料。資料包說明排盤口徑，缺少哪個層級，AI 會如實說明，不會以常識補填。</p>
  <h2>奇門排盤常見誤解</h2>
  <p>第一，以為隨時排一局結果相同：奇門一時一局，同一個時辰內盤局相同，換一個時辰則全局重排，因此「所占之事」須在起念之時起局。第二，將值符值使視為吉凶答案：值符代表事情主氣所在、值使關乎人事走動，是讀盤入口，不是結論，吉凶須看用神落宮的門星神組合。第三，將拆補與置閏視為對錯之爭：兩者是不同定局法，各流派各有傳承，本工具固定使用拆補並於資料包中說明，方便核對。</p>
  <h2>常見問題</h2>
  <div class="faq-item"><h3>Q1　奇門遁甲需要輸入出生時間嗎？</h3><p>不需要。奇門以起局時刻定盤，不需要出生資料；若希望在盤中對照自身，可選填出生年作為年命參照。</p></div>
  <div class="faq-item"><h3>Q2　什麼是值符值使？</h3><p>值符是當旬旬首所配之九星，代表事情的主氣所在；值使是同宮的八門，多與人事及行動相關。工具將列明兩者所落之宮位。</p></div>
  <div class="faq-item"><h3>Q3　拆補定局是什麼？</h3><p>拆補以起局日的符頭（最近的甲日或己日）地支定上中下元：子午卯酉為上元、寅申巳亥為中元、辰戌丑未為下元，再按節氣查局數。本工具固定使用拆補，並於資料包中說明。</p></div>
  <div class="faq-item"><h3>Q4　為何中五宮沒有門星神？</h3><p>轉盤奇門中五宮不安門星神，寄於坤二宮；天禽星寄天芮。工具在中宮及資料包中均會標明「寄坤二宮」。</p></div>
  <div class="faq-item"><h3>Q5　將資料包貼給 AI 之後，AI 會否隨意補充？</h3><p>資料包附有使用規則：以資料包為唯一盤面事實、不得改局重排、未列出的層級須直接說明。實測主流模型均會遵從；若 AI 輸出的盤面與資料包不一致，以資料包為準。</p></div>
  <div class="faq-item"><h3>Q6　此工具需要付費嗎？</h3><p>不需要。奇門排盤與 AI 資料包全部免費，免登入、免安裝。</p></div>
</section>

<footer>
  本工具只輸出可核對的盤面結構，不出吉凶斷語；解讀屬命理參考，不構成專業意見。<br>
