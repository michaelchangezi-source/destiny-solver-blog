...
// Convert ISO (with +08:00 or Z) to datetime-local value "YYYY-MM-DDTHH:MM"
function isoToLocal(iso: string): string {
  if (!iso) return ''
  if (iso.includes('+08:00') || iso.includes('+0800')) return iso.slice(0, 16)
  try {
    const d = new Date(iso)
    const hkt = new Date(d.getTime() + 8 * 60 * 60 * 1000)
    return hkt.toISOString().slice(0, 16)
  } catch { return '' }
}
...
export default function EditArticle() {
  ...
  useEffect(() => {
    fetch(`/api/admin/articles/${slug}`)
      ...
      .then(data => {
        ...
        setPublishedAt(isoToLocal((fm.publishedAt as string) || ''))
        setCoverImage((fm.coverImage as string) || '')
  ...
  async function handleBodyImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setError('')
    try { setInsertPath(await uploadFile(file)) }
    catch (err) { setError(err instanceof Error ? err.message : '上傳失敗') }
    finally { setInsertUploading(false); e.target.value = '' }
  }
  ...
  function handleInsertImage() {
    ...
    setInsertPath(''); setInsertWidth(''); setShowInsert(false)
    setTimeout(() => {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/admin/articles/${slug}`, {
        ...
        body: JSON.stringify({ content: buildMarkdown() }),
      })
      if (res.ok) { router.push('/admin/dashboard') }
      else { const d = await res.json(); setError(d.error || '更新失敗') }
    } catch { setError('網絡錯誤') }
  ...
  if (loading) return <div className="max-w-4xl mx-auto p-6"><p className="text-white/50">載入中...</p></div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input type="text" value={coverImage} onChange={e => setCoverImage(e.target.value)}
                className={`${inputCls} flex-1 min-w-0 text-sm`} placeholder="/images/covers/filename.png" />
            ...
            {coverImage && <img src={coverImage} alt="封面預覽" className="mt-2 h-20 w-full object-cover rounded-lg opacity-80" />}
          </div>
        </div>
