...
const faq = [
  {
    q: '合盤分析的是什麼？',
    a: '合盤會分別排出雙方的四柱八字，再比對兩人天干地支之間的互動關係：天干五合、地支六合、地支三合、地支六沖、地支六害、地支相破、地支三刑（含自刑），並特別列出雙方日支的對應關係。',
  },
  {
    q: '三刑與自刑代表什麼？',
    a: '三刑分為寅巳申（恃勢之刑）、丑戌未（無恩之刑）、子卯（無禮之刑）三組，代表雙方在對應柱位容易出現角力、拉扯或言語衝突，需要刻意克制與體諒；自刑則是同一地支相遇（辰辰、午午、酉酉、亥亥），與伏吟同時出現，代表兩人在同一位置的能量過於相似，容易一起卡在同一個盲點。',
  },
  {
    q: '日支關係為什麼特別重要？',
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 2, name: '八字合盤', item: `${SITE_URL}/compat` },
  ],
}
        ...
        <div className="max-w-3xl mx-auto mt-16 space-y-10">
          <section>
            ...
            <p className="text-[#5A5247] text-sm leading-relaxed">
              輸入雙方的出生年、月、日、時之後，系統會分別排出兩人的四柱八字，再比對雙方天干地支之間的互動關係：天干五合、地支六合、地支三合、地支六沖、地支六害、地支相破、地支三刑。天干五合是天干之間兩兩相合的關係，例如甲己合、乙庚合，反映雙方性情上的吸引與牽絆。地支六合是地支兩兩相合，例如子丑合、寅亥合，象徵關係緊密與專注，但過度的合亦可能演變成一種羈絆。地支三合是三個地支聚合成一個新五行局，力量遠勝六合，象徵雙方合作能匯聚成更大的能量場。地支六沖是地支之間相對沖突的關係，例如子午沖、卯酉沖，代表變動與張力。地支六害與地支相破則反映較隱性的耗損與阻滯。地支三刑分為寅巳申、丑戌未、子卯三組，代表雙方在對應柱位容易出現角力與拉扯；若雙方同一柱位落在同一地支（辰辰、午午、酉酉、亥亥），則同時構成伏吟與自刑，代表兩人在該位置的能量高度相似。工具亦會特別列出雙方的日支關係，因為日支對應日主自身的宮位，是最直接反映兩人相處模式的一組對照。
            </p>
          </section>
          ...
          <section>
            ...
            <p className="text-[#5A5247] text-sm leading-relaxed">
              合盤結果分為兩部分：雙方命盤與干支互動總覽。雙方命盤並列顯示兩人的四柱，方便直接對照。干支互動總覽首先呈現日支關係，即兩人日柱地支之間的對應，這一組關係最直接反映日常相處的動態；其餘互動則按類型分組列出，並標示「合」與「沖害破」的數量對比，合的數量偏多，代表雙方能量傾向聚攏與吸引；沖害破的數量偏多，則代表雙方能量傾向張力與變動。每一組互動旁邊都附有分層說明：同一種干支組合落在不同柱位，代表的分量並不相同，日柱互動的權重明顯高於年柱，因為日柱最貼近雙方的相處核心。每一組互動亦附有雙向視角，分別說明甲方與乙方在自己柱位上感受到的意義，因為同一組干支關係，對雙方而言意義未必相同。如果日柱地支落在互動之中，工具會特別標示「日柱」，提示這組互動與雙方相處模式關係更直接。
            </p>
          </section>
