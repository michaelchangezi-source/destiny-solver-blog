...
export default function LatestCard({ article }: { article: ArticleMeta; priority?: boolean }) {
  ...
  return (
    <Link href={`/articles/${article.slug}`} className="group block">
      <div className="rounded-lg overflow-hidden bg-[#FFFFFF] border border-[color:var(--border-card)] shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-lift)] hover:border-[#B23E26] hover:bg-[#FBF7EE] transition-colors duration-200">
        {/* 分類五行強調色條（C1 快速掃讀辨識） */}
        ...
        <div className="p-5">
          <h3 className="text-[#2B241C] font-bold text-base leading-snug mb-2 group-hover:text-[#B23E26] transition-colors line-clamp-2 break-keep">
            {article.title}
          </h3>
